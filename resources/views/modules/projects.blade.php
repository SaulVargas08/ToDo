...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box;}
        ...
        .wrapper{max-width:800px;margin:0 auto;padding:32px 20px 60px;}
...
<body>
    <div class="wrapper">
        ...
        <div class="status" id="status">Crea, actualiza o elimina proyectos.</div>
        <div class="grid">
            <div class="panel">
                <h2>Crear</h2>
                <form id="formCreate">
    ...
    <script>
        ...
        document.getElementById('formCreate').addEventListener('submit', async (e)=>{
            ...
            try{ await apiRequest('POST','/api/projects',payload); statusEl.textContent='Proyecto creado'; e.target.reset(); }
            catch(err){ statusEl.textContent=err.message; }
        });
        document.getElementById('formUpdate').addEventListener('submit', async (e)=>{
            e.preventDefault();
            const fd=new FormData(e.target); const id=fd.get('id'); const payload=Object.fromEntries(fd.entries()); delete payload.id;
            try{ await apiRequest('PUT',`/api/projects/${id}`,payload); statusEl.textContent='Proyecto actualizado'; }
            catch(err){ statusEl.textContent=err.message; }
        });
        document.getElementById('formDelete').addEventListener('submit', async (e)=>{
            e.preventDefault();
            const id=new FormData(e.target).get('id'); if(!id) return;
            try{ await apiRequest('DELETE',`/api/projects/${id}`); statusEl.textContent='Proyecto eliminado'; }
            catch(err){ statusEl.textContent=err.message; }
        });
    </script>
</body>
